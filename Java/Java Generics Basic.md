...
&emsp;[5. 约束和局限性](#5-约束和局限性)   
&emsp;&emsp;[5.1 不能用基本类型实例化类型参数](#51-不能用基本类型实例化类型参数)   
&emsp;&emsp;[5.2 运行时的类型查询只适用于原始类型](#52-运行时的类型查询只适用于原始类型)   
&emsp;&emsp;[5.3 不能创建参数化类型的数组](#53-不能创建参数化类型的数组)   
&emsp;&emsp;[5.4 Varargs 警告](#54-varargs-警告)   
&emsp;&emsp;[5.5 不能实例化类型变量](#55-不能实例化类型变量)   
&emsp;&emsp;[5.5 不能构造泛型数组](#55-不能构造泛型数组)   
&emsp;&emsp;[5.6 泛型类静态成员类型参数无效](#56-泛型类静态成员类型参数无效)   
&emsp;&emsp;[5.7 不能抛出或捕获泛型类对象](#57-不能抛出或捕获泛型类对象)   
&emsp;&emsp;[5.8 注意擦除后的冲突](#58-注意擦除后的冲突)   
&emsp;[6. 泛型类的继承](#6-泛型类的继承)   
